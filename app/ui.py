...
class LoginDialog(tk.Toplevel):
    ...
    def novo_usuario(self):
        ...
        if novo and novo.strip():
            ...
            try:
                db.add_usuario(novo)
                logger.info("Novo usuário criado: %s", novo)
                self.usuarios = db.get_all_usuarios()
                ...
                logger.error("Erro ao criar usuário '%s': %s", novo, e, exc_info=True)
    ...
    def excluir_usuario(self):
        try:
            ...
            if messagebox.askyesno("Excluir", f"Deseja realmente EXCLUIR permanentemente o usuário '{selecionado}'?", parent=self):
                ...
                if self.usuarios:
                    self.listbox_usuarios.selection_set(0)
                logger.info("Usuário excluído: %s", selecionado)
...
class TabNumerador(ttk.Frame):
    ...
    def action_salvar(self):
        ...
        try:
            ...
            logger.info("Registro salvo: %s Nº %s por %s", self.tipo_db, num_str, usuario)
        except Exception as e:
            logger.error("Erro ao salvar registro %s: %s", self.tipo_db, e, exc_info=True)
    ...
    def action_excluir(self):
        ...
        if messagebox.askyesno("Excluir Documento", f"ALERTA! Tem certeza que deseja apagar DEFINITIVAMENTE o registro {self.var_numero.get()}?"):
            try:
                ...
                logger.info("Registro excluído: %s Nº %s", self.tipo_db, numero_apagado)
                self.action_novo()
                self.load_data()
            except Exception as e:
                logger.error("Erro ao excluir registro: %s", e, exc_info=True)
    ...
    def action_abrir_pasta(self):
        try:
            ...
            logger.error("Erro ao abrir pasta de documentos: %s", e)
    ...
    def action_abrir_numerador(self):
        try:
            ...
            logger.error("Erro ao gerar/abrir DOCX para %s: %s", self.tipo_db, e, exc_info=True)
...
class TabAuditoria(ttk.Frame):
    ...
    def load_data(self):
        ...
        try:
            ...
            logger.warning("Falha ao carregar histórico de auditoria")
    ...
    def auto_sync_loop(self):
        try:
            ...
            try: self.tab_auditoria.load_data()
                ...
                logger.debug("Falha ao recarregar auditoria no auto_sync: %s", e)
        except Exception as e:
            logger.error("Erro no auto_sync_loop: %s", e, exc_info=True)
        self.root.after(60000, self.auto_sync_loop)
        
    def auto_backup_loop(self):
        try:
            db.fazer_backup()
        except Exception as e:
            logger.error("Erro no auto_backup_loop: %s", e, exc_info=True)
        ...
        if messagebox.askyesno("Confirmar", "Deseja realizar o Logo-ff do sistema?"):
            ...
            try:
                ...
                if os.path.exists(bat_path):
                    ...
                    logger.info("Sistema reiniciado via run.bat")
                else:
                    # Fallback para script local
                    import subprocess
                    subprocess.Popen([sys.executable, sys.argv[0]])
                    logger.info("Sistema reiniciado via subprocess")
            except Exception as e:
                logger.error("Erro ao reiniciar sistema após troca de usuário: %s", e)
            sys.exit()
